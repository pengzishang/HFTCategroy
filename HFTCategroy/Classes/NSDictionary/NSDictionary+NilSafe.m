...
- (NSDictionary *)deleteAllNullValue {
    ...
    for (NSString *keyStr in self.allKeys) {
        if ([[self objectForKey:keyStr] isKindOfClass:[NSNull class]] || [self objectForKey:keyStr] == nil) {
...
- (void)gl_setObject:(id)anObject forKey:(id<NSCopying>)aKey {
    ...
    [weakSelf gl_setObject:anObject forKey:aKey];
    [self deleteAllNilValue];
}
...
- (void)deleteAllNilValue {
    for (NSString *keyStr in self.allKeys) {
        if ([[self objectForKey:keyStr] isKindOfClass:[NSNull class]] || [self objectForKey:keyStr] == nil) {
            [self removeObjectForKey:keyStr];
        }
    }
}
