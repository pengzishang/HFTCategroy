...
#define kDefautImageSize (1600 * 1200)
...
- (NSData *)imageCompressForSpecifyKB:(NSInteger)specifyKB {
    ...
    if (resultData.length > stdSize) {
        ...
        NSLog(@"try.rate = %.1f, try.size = %lu", currentRate, (unsigned long)resultData.length);
        ...
        while (0 < currentRate && currentRate < 1) {
            ...
            NSLog(@"try.rate = %.1f, try.size = %lu", currentRate+rateOffset, (unsigned long)tryData.length);
            if (rateOffset < 0) {
                //如果不满足条件，则保存起来，并继续尝试
    ...
    NSLog(@"ret.rate = %.1f, ret.size = %lu", currentRate, (unsigned long)resultData.length);
    return resultData;
}
